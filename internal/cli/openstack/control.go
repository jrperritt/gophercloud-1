...
func Action(ctx *cli.Context, cmd interfaces.Commander) error {
	...
	*/
	...
	if p, ok := cmd.(interfaces.PipeCommander); ok {
		lib.Log.Debugln("cmd implements PipeCommander")
		p.SetConcurrency(cmd.Context().Int("concurrency"))
		lib.Log.Debugln("p.Concurrency() : ", p.Concurrency())
	}
