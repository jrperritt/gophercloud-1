...
func (cache Cache) GetCacheValue(cacheKey string) (CacheItemer, error) {
	err := cache.all()
	if err != nil {
...
func (cache *Cache) SetCacheValue(cacheKey string, cacheItemer CacheItemer) error {
	// get cache items
	err := cache.all()
	if err != nil {
		return err
...
func (cache *Cache) StoreCredentials() error {
